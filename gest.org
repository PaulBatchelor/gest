...
Applying a state to gest can be done with =apply_state=.

#+NAME: static_funcdefs
#+BEGIN_SRC c
static void apply_state(gest_d *g, gest_state *s);
#+END_SRC
...
static void apply_state(gest_d *g, gest_state *s)
{
    ...
    if (g->curtarget->next != NULL) {
        g->nxtval = g->curtarget->next->value;
    } else {
        g->nxtval = g->curval;
    }
...
Saving a state from gest can be done with =save_state=.

#+NAME: static_funcdefs
#+BEGIN_SRC c
static void save_state(gest_d *g, gest_state *s);
#+END_SRC
...
if (reset) {
    ...
    if (g->beat >= limit) {
        /* TODO: trigger targetondeck flag here */
        /* node of next phrase should already be found at this point */
        gest_node *next;
...
out = g->phs;
#+END_SRC
** Updating Targets (Currently Subject to change)
If any targets were slotted to be updated in the previous
call, update them now, then clear the flag.

---

Moving forward, the =targetondeck= flag will not only be
...
if (g->targetondeck) {
    ...
    /* TODO: apply next state here */

    /* TODO: find next node, save state in nxtstate */
}
#+END_SRC
...
#+NAME: static_funcdefs
#+BEGIN_SRC c
static gest_node* find_next_node(gest_d *g,
                                 gest_node *top,
                                 gest_state *state);
#+END_SRC
...
Eventually, =find_next_node= will become an operation
will only temporarily manipulate the state of =gest_d=,
before returning it back to what it was before. This
is needed because the next node is found immediately
after the current node is set.
                                 ...
                                 gest_state *state)
{
    ...
    while (next == NULL) {
        if (top == g->phrase_selected->top) {
            ...
            if (top->next == NULL) {
                ...
                /* TODO this should be a value returned */
                /* TODO phrase *should* be found here and stored in state */
                g->please_wait = 1;
                break;
            } else {
    ...
    /* TODO restore old numerator and denominator */
    /* TODO next node should somehow be stored in state */

    save_state(g, state);

    /* TODO this should return the pleasewait flag */
    return next;
}
#+END_SRC
...
#+NAME: find_next_node
#+BEGIN_SRC c
g->nxtnode = find_next_node(g, g->curnode, &g->nxtstate);
if (g->nxtnode != NULL) {
