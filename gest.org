...
#+END_SRC
** Squawk Level
The =squawk= is a value that determines how
much verbosity is printed on screen. It can be
set with =gest_squawk=. The higher the number,
...
#+END_SRC
** Spillover Flag and Tolerance
When =spillover= flag is engaged, it will enable phrases
...
The =tolerance= is the amount which is allowed to spillover.
...
The control flow of traversal starts large (checking
...
#+NAME: reset_and_wait_for_next_phrase
#+BEGIN_SRC c
...
Alternatively, if there are still nodes in the top-level
node, go there instead.

TODO: do we need to handle edge case when next node is
monoramp with a modifier greater than 1? We are doing
that when finding sibling nodes.

#+NAME: goto_next_child_in_top_node
#+BEGIN_SRC c
next = top->next;
...
It may be a little bit confusing to see the parent node
...
There is an edge case when selected nodes do actually
...
#+NAME: find_next_node_with_target
#+BEGIN_SRC c
next = top->next;
...
=spillover= can be applied. This is a special mode that
...
a value called =tolerance=. Tolerance is a value between 0
...
The spillover flag gets turned on during the beat checkin
...
point, the =please_wait= flag should be turned on. When
...
In Spillover mode, beat check-in is skipped entirely;
there is hardly a need for it, so it is ignored.
** edge case: please_wait flag in beat check-in
Here's a weird edge case: the phrase finishes and
the =please_wait= flag is set one sample before the clock
reset. When this happens, a single sample discontinuity will
be returned. Why?

When computing the ramptree, beat-checkin happens before
the =please_wait= flag is checked. In most scenarios,
=please_wait= is unset by the time it reaches here. But,
...
=please_wait= in the beat check-in when the check-in
...
about undershoot. The local variable =phs= gets set to be 1.
After some re-init for the next node, this value of gets
returned.

#+NAME: please_wait_in_undershoot
#+BEGIN_SRC c
if (g->please_wait) {
    if (g->squawk > 1) {
...
When the =spillover= flag gets turned on, =please_wait= will
need to be checked too. Normally, the program is used to
waiting a few samples before =please_wait= is set. But in
this edge case, it will only spill over into the next
sample.

In this situation, the internal phasor value is set to be
1.0 in addition to returning 1.0, which is needed for
the next sample iteration.

#+NAME: please_wait_in_spillover
#+BEGIN_SRC c
if (g->please_wait) {
    if (g->squawk) {
