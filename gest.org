...
TODO: =shouldiwait= works under the assumption that the
next phrase will always be different from the currently
selected phrase. But this isn't necessarily true. A phrase
that loops on itself will never wait. There needs to be
better logic in place for handling this. Some sort of
flag would probably be the clearest way to indicate that
a new phrase is starting. Maybe =pleasewait= is all that is
needed? More thought required.
...
size, and array of phrases. The parent phrase is stored
here as well.
*** Allocation
