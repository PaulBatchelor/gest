...
TODO: I can't
remember why we are passing in the parent node and not
the top node. As of writing, I also had to make a change
to compensate for an edge case for when the rightmost node
was a monoramp with a modifier >1 (similar
to edge applied when recursively finding
the next node). I guess this was never an
issue before?
