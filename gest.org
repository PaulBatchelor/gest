...
#+NAME: funcs
#+BEGIN_SRC c
static SKFLT gliss(gest_d *g, SKFLT x, SKFLT y, SKFLT a, void *ud)
{
    if (a < 0.5) {
        a = 0;
    } else {
...
=smallgliss= does this, but smaller scale.
...
#+NAME: funcs
#+BEGIN_SRC c
static SKFLT smallgliss(gest_d *g, SKFLT x, SKFLT y, SKFLT a, void *ud)
{
    if (a < 0.97) {
        a = 0;
    } else {
...
#+NAME: funcs
#+BEGIN_SRC c
static SKFLT exponential(gest_d *g, SKFLT x, SKFLT y, SKFLT a, void *ud)
{
    SKFLT *s;

    s = ud;

    a = (1.0 - exp(a * (*s))) / (1 - exp(*s));
    return (1 - a)*x + a*y;
}
...
void gest_behavior_exponential(gest_d *g, SKFLT slope)
{
    ...
    g->curtarget->ud = gest_alloc(g, sizeof(SKFLT));
    s = (SKFLT *)g->curtarget->ud;
    *s = slope;
}
#+END_SRC
** Bezier
